    ...
    void (async () => {
      ...
      try {
        const token = isAuthenticated
          ...
          : undefined;
        const response = await fetchDoctorDashboard(Number(patientId), token);
        setPayload(response as Record<string, unknown>);
      } catch (error) {
        setPayload(null);
        setErrorText(error instanceof Error ? error.message : "Failed to fetch dashboard.");
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      <div className="space-y-2">
        ...
        <h1 className="text-2xl font-semibold">Doctor Dashboard API View</h1>
        <p className="text-sm text-muted-foreground">
          Fetches backend data from `GET /doctor/dashboard/{`{patient_id}`}`.
        </p>
      </div>

      {!isAuthenticated && (
        <Button onClick={() => loginWithRedirect()}>Sign In as Doctor</Button>
      )}

      <form onSubmit={loadDashboard} className="space-y-3 rounded-lg border p-4">
        <div className="max-w-xs space-y-2">
          <Label htmlFor="doctor_patient_id">patient_id</Label>
          <Input
            id="doctor_patient_id"
            value={patientId}
            onChange={(event) => setPatientId(event.target.value)}
          />
        </div>
        <Button type="submit" disabled={isLoading}>
          {isLoading ? "Loading..." : "Fetch Dashboard"}
        </Button>
      </form>
